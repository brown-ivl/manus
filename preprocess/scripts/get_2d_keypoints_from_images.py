...
# parser.add_argument("--checkpoint", default="AlphaPose_mp/pretrained_models/fintune_wholebody_1000hand.pth", 
...
image_base = os.path.join(base_path, "images", "image")
...
all_frames = glob(os.path.join(image_base, cam_names[0], "*.jpg"))
chosen_frames = [int(os.path.basename(f).split(".")[0]) for f in all_frames]
...
for idx in chosen_frames:
    ...
    for i in range(len(params)):
        ...
        frames_dict[idx][cam_name] = path

for idx in tqdm(chosen_frames):
    frames = frames_dict[idx]
